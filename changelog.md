...
-  Improve error handling significantly
 ...
 -  Added AggregateError class. If a unresolved and by node_redis rejected command has no callback and
    ...
    to a single error that is emitted in that case.
...
-  Fixed silently rejecting unresolved commands without callback. An AbortError / AggregateError is emitted in that case from now on.
